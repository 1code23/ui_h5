...
export default {
  ...
  methods: {
    ...
    handleDownload() {
      if(this.download_flg==2){
        window.open(this.download_url)
      }else{
        this.$refs.multTableRef.downloadTable(this.title);
      }
    },
    async getTableData(params) {
      ...
      this.loading = true;
      const res = await this.$commonApi.AppPreInstallData(_params);
      this.loading = false;
      let list = [];
      if (res.code == 200) {
        const data = res.data;
        ...
        this.download_flg=data.download_flg
